<template>
  <div class="home">
    <div class="container">
        <!-- retrieve the user infos : name and image  -->
      <div v-if="currentUser" class="infoUser col-12">
        <div class="own" v-if="userId == currentUser.id">
          <label for="image">
            <img class="ppChange" v-bind:src="currentUser.pp" alt="pp" srcset=""/>
            <img class="file" src="../assets/default-user.jpg" alt="" srcset=""/>
          </label>
          <input @change="modifPP" type="file" name="image" id="image" style="display: none"/>
        </div>
        <div v-else>
        ...
        <div class="text-user">
          ...
          <p v-if="currentUser.desc" class="metier">
            <strong>Un mot sur moi :</strong> {{ currentUser.desc }}
          </p>
        </div>
      </div>
        <!-- END retrieve the user infos : name and image  -->

        <!-- START retrieve the posts  -->
      <div v-if="first">
        <div v-for="post in posts" :key="post.postId" class="d-flex justify-content-center">
          <div class="bg-white border mt-2 mb-2 col-sm-8 col-lg-6 posts">
            ...
            <!-- card of the post : img, name, date, delete  -->
            <div>
              <div class="d-flex flex-row justify-content-between align-items-center p-2 border-bottom infopost">
                <div class="d-flex flex-row align-items-center px-2">
                  ...
                  <div class="d-flex flex-column flex-wrap ml-2">
                    <span class="font-weight-bold nomUser">{{ post.prenom }} {{ post.nom }}</span>
                    <span class="text-black-50">Posté le {{ formatDate(post.date) }}</span>
                  </div>
                  <!-- cross to delete the post  -->
                  <img class="deletePost" src="../assets/checkmark.svg" alt="supprimer la publication"
            ...
            <!-- like and comment icons counter  -->
            <div class="react">
            ...
            <!-- END like and comment icons counter  -->
              ...
              >
                <div class="pp" v-if="post.postId === comment.postId">
                  <router-link :to="{ name: 'user', params: { userId: comment.authorId } }">
                    <img class="rounded-circle pp2" v-bind:src="comment.pp" alt="" srcset=""/>
                  </router-link>
                </div>
