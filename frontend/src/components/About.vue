  ...
  <div class="UserProfil">
    ...
    <div v-if="approuvedConnexion" class="background d-flex flex-column">
        <form id="profil" @submit.prevent = saveUser()>
            ...
            <input id="lastname" ref="nom" type="text" title="Renseignez votre nom" placeholder="Nom" :value="userProfil.lastname">
            <input id="firstname" ref="prenom" type="text" title="Renseignez votre prénom" placeholder="Prénom" :value="userProfil.fisrtname">
            <input id="email" ref="email" type="email" title="Renseignez votre email" placeholder="E-mail (*)" :value="userProfil.email" required>
            <input id="position" ref="poste" type="text" title="Renseignez votre poste" placeholder="Poste occupé" :value="userProfil.position">
...
export default {
  ...
  mounted(){
    if(this.approuvedConnexion === true) {
      ...
      let decodedToken = jwt.verify(token, process.env.VUE_SECRET_TOKEN_KEY);        // on décode le token avec la fonction verify qui prend le token et la clé secrète
